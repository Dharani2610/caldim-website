...
async function capture() {
  ...
  for (const item of pagesToCapture) {
    // 1. Dark Theme
    {
      ...
      await page.screenshot({ path: path.join(artifactsDir, `before-${item.name}-dark.png`), fullPage: false });
      await context.close();
    }
  }

  await browser.close();
  console.log('Successfully captured clean BEFORE dark screenshots');
}
