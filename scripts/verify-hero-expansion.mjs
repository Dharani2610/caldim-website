...
const outDir = 'C:\\Users\\USER\\.gemini\\antigravity-ide\\brain\\7aa5c920-ce2c-4da4-99e2-32c40072eaa4';
...
async function capture() {
  ...
  // Frame 0: t ~ 0.35s - Foundations and Base Plates
  await wait(350);
  ...
  // Copy before/after screenshots to artifact dir
  await page.screenshot({ path: path.join(outDir, 'hero-3d-after-375.png') });
  await page.setViewportSize({ width: 1440, height: 900 });
  await wait(300);
  await page.screenshot({ path: path.join(outDir, 'hero-3d-after-1440.png') });
    ...
    return new Promise((resolve) => {
      ...
      function tick() {
        ...
        if (now - start < 1500) {
          ...
          resolve({ avgFps: avgFps.toFixed(1), avgFrameTime: avgFrameTime.toFixed(2), frames });
        }
      }
      requestAnimationFrame(tick);
    });
  });
  console.log(`Performance metric: ${fpsData.avgFps} FPS, avg frame time: ${fpsData.avgFrameTime} ms (${fpsData.frames} frames measured)`);

  // Reduced motion emulation fallback check
  ...
  // Test responsive overflow across standard break points (320px to 1920px)
